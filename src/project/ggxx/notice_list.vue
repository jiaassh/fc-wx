  ...
  <div>
    ...
    <div>
      <Scroller style="margin-top:-44.8px;" :on-refresh="onRefresh" :on-infinite="onInfinite">
        ...
        <load-more v-if="noticelists.length===0" :show-loading="false" tip="暂无公告信息" background-color="#efeff4"></load-more>
        <group v-for="(item,index) in noticelists" key="item" @click.native="goDetail(item.noticeid.val)" gutter="0">
          <div style="padding:.53rem .4rem;">
            <div style="width:9rem;font-size:.43rem;overflow: hidden; text-overflow:ellipsis; white-space: nowrap;margin-bottom:.4rem;">
              {{item.title.val}}
            </div>
            <div style="font-size: .32rem;"><span style="color:#ad7de7;">{{item.isnotice.val==='1'?'社区公告':'社区活动'}}</span><span style="color:#999;margin-left:.5rem;">{{ getDate(item.startdate.val) }}</span></div>
          </div>
        </group>
      </Scroller>
    </div>
...
<script>
  ...
  import {
    Swiper,
    Group,
    Cell,CellBox,SwiperItem,LoadMore
  } from 'vux'
  ...
  export default{
    ...
    components:{
      Swiper,Loading,Group,Cell,CellBox,SwiperItem,Scroller,LoadMore
    },
    methods:{
      getDate (datetime) {
...
//            alert(year + '-' + month + '-' + day + '  ' + hour + ':' + minute)
      ...
      onInfinite(done){
        ...
        if(!this.lastid){
          ...
          let vid = userProfile.vid?userProfile.vid:sessionStorage.getItem("vid");
          this.$http.get(service_url+"/o2o/notice/wx/getlist?lastid="+this.lastid+"&vid="+vid).then((data) => {
