  ...
  <div>
    <section v-if="!showLoading" class="shop_container">
      ...
      <section style="margin-top:45px;">
        <img style="width: 100%;height:100%;" src="../../../static/img/banner1.jpg" />
      </section>
        ...
        <section class="buy_cart_container">
          <section @click="toggleCartList" class="cart_icon_num">
            ...
            <div class="bld_cart_num">
              <div :style="{color: totalPrice? 'red':'#999'}" :class="{bld_food_price:totalPrice}">
                <span style="font-size:.5rem;">￥</span>{{totalPrice?totalPrice:"未选购商品"}}</div>
            </div>
          </section>
...
export default {
  ...
  methods: {
    ...
    getMenuList() {
      ...
      this.$http.get("http://192.168.0.145:8090/o2o/shop/wx/selectOtherFreeStore.do").then((data) => {
        console.log(data)
        ...
        if (goods_id == this.cartList1[i].goods_id) {
          for (let j = 0; j < this.cartList1[i].models.length; j++) {
            if (model_id == this.cartList1[i].models[j].modelid) {
              this.cartList1[i].models[j].len -= 1;
              this.totalNum -= 1;
              console.log(this.cartList1[i].models[j].price,123)
              this.totalPrice -= parseFloat(this.cartList1[i].models[j].price);
