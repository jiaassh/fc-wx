  ...
  <div>
    ...
    <div>
      ...
      <group v-if="route_state===2" title="订单评价">
        <cell title="评分">
          <rater v-model="eva_score" active-color="#FF9900" :margin="4"></rater>
        </cell>
        <x-textarea :max="100" v-model="eva_content" placeholder="请输入详细内容" :show-counter="false"></x-textarea>
      </group>
      <group v-if="route_state===3" title="订单评价">
        <cell title="评分">
          <rater v-model="eva_score" active-color="#FF9900" :margin="4" disabled></rater>
        </cell>
      ...
      <div style="padding:10px 10px 20px;">
        <div style="padding:0 15px 30px 15px;font-size:12px;color:#ad7de7;">提前15分钟可以入场</div>
  ...
  export default{
    ...
    mounted(){
      ...
      if(this.route_type===1){
        ...
        this.this_sum=this.list.this_sum.toFixed(2);
    ...
    methods: {
      ...
      goNext(){
        if(!this.list_len){
          ...
          this.confirm_content="您当前没有会员卡或会员卡已失效，请重新办理！";
...
//        this.$router.push({path:'reserve_success',query:{route_type:1}});
      ...
      getOrderList(){
        ...
        if(service_url){
          this.$http.post(service_url+ "/o2o/club/wx/preDetail",item).then((data)=>{
            ...
            self.this_sum=parseFloat(data.body.data.price).toFixed(2);
      ...
      get_evaluate(){
          ...
          this.$http.post(service_url+"/o2o/club/wx/preEvaluate",item).then((data)=>{
              if(data.body.status===0){
                self.eva_score=Number(data.body.data.eva_score) ;
                console.log(self.eva_score,'score')
