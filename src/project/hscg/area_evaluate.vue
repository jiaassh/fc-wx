<template>
  <div>
    <group gutter="0">
      <cell title="评分">
        <rater v-if="route_type===1" v-model="eva_score" slot="value" active-color="#FF9900" :margin="4"></rater>
        <rater v-else v-model="eva_score" slot="value" active-color="#FF9900" :margin="4" disabled></rater>
      </cell>
    </group>
    <group gutter="0">
      <x-textarea v-if="route_type===1" :max="100" v-model="eva_content" :placeholder="placeholder"></x-textarea>
      <div v-else style="padding:10px 15px 10px 0;display:flex;margin-left:15px;border-top:1px solid #ececec;">
        ...
        <span style="height:70px;">{{eva_content}}</span>
      </div>
    </group>
