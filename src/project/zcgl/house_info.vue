<template>
  <div id="houseinfo">
    ...
    <div>
      ...
      <!-- <div v-if="index===0" style="line-height:1.2rem;padding-left:0.4rem">房屋出售</div> -->
      <!-- <div v-if="index===1" style="line-height:1.2rem;padding-left:0.4rem">房屋出租</div> -->
      <!-- <div v-if="index===2" style="line-height:1.2rem;padding-left:0.4rem">房屋托管</div> -->
      <div v-if="index===1">
        <group gutter="0">
          <x-input class="font_14" labelWidth="6em" title='房屋地址' v-model="position"></x-input>
          ...
          <div style="padding:10px 15px 10px 0;margin-left:15px;border-top:1px solid #ececec;" class="font_14">
            ...
            <input v-model="min_price" class="weui_input" @blur="checknum(min_price)" type="text" placeholder="最低价"/>~
            <input v-model="max_price" style="margin-left:10px;"  @blur="checknum1(max_price)" class="weui_input" type="text" placeholder="最高价"/>
            元
          </div>
          <x-input class="font_14" labelWidth="6em" title='联系电话' v-model="phone"></x-input>
...
<script>
  ...
  export default{
    ...
    mounted(){
      this.assets_project_id=this.$route.query.assignid
      console.log(this.assets_project_id,'assets_project_id')
    ...
    methods:{
      ...
      getList(){
          let self = this;
          if(service_url){
            this.$http.get(service_url+"/o2o/asset/wx/houseinfo.do?houseid="+userProfile.houseid).then((data)=>{
              self.data1=data.body.data;
              self.position=data.body.data?data.body.data.position:"";
              self.phone=data.body.data?data.body.data.phone:"";
            })
          }else{
            this.$http.get("/api/houseinfo").then((data)=>{
              self.position=data.body.data.position;
              self.phone=data.body.data.phone;
            })
          }
      ...
      validate(key){
        if(key===1){
          ...
          if(parseFloat(this.max_price)<=parseFloat(this.min_price)){
            this.confirm_content="最高价应该大于最低价";
            this.confirm_show=true;
            return false;
          }
          if(!this.max_price){
      ...
      submit1(){
        ...
        const items={
          "data":{
            "assets_project_id":this.assets_project_id,
            "address":this.position,
            "phone":this.phone,
            "area":parseFloat(this.acreage), //面积
