<!--我的钱包，包含1、物业缴费  2、消费记录   3、充值   4、设置密码   5、绑定银行卡   6、开票信息入口-->
<template>
  <div>
    <!--<loading v-if="loading" style="text-align: center"></loading>-->
    <div>
      <div style="background-color:white;color:#ad7de7;padding:30px 15px">
        <div style="float:left">当前账户余额</div><div style="float:right;">限额5W</div>
        <div style="clear:both;text-align: center"><span style="font-size:30px;">￥{{total_price}}</span>元</div>
      </div>
      <group gutter="10px">
        <cell v-for="item in cell_list" :key="item.url" :link="item.url" :title="item.title" is-link>
...
<script>
  ...
  export default{
    ...
    mounted(){
      ...
      /*从localStorage中获取是否已经进入过钱包页面*/
      let wallet_first=window.localStorage.getItem("wallet_first_1");
      let self = this;
      ...
      if(service_url){
        ...
        this.$http.get(service_url+"/o2o/assetsv2/wx/my_wallet.do").then((data)=>{
          if(data.body.status===0){
            /*如果data.body.data存在则当前账户余额去获取并且保留2位小数，否则设置为0.00*/
            self.total_price=data.body.data?parseFloat(data.body.data.total_price).toFixed(2):"0.00";
            /*将现有价格拼入物业缴费中*/
            this.cell_list[0].url=this.cell_list[0].url+"?pay="+self.total_price;
            /*如果是第一次进入则进入开通钱包页面，如果不是则进入到充值页面*/
            this.cell_list[2].url=wallet_first?this.cell_list[2].url+"&pay="+self.total_price:"open_wallet?type=2";
            /*如果已有密码则进入到修改密码，如果没有密码则进入到新增密码*/
            this.cell_list[3].url=data.body.tag===1?"wallet_code?mark=2":"open_wallet?type=1";
            /*如果进来的是租户身份则没有充值和设置密码两项功能*/
            if(enter_mark===2){
              this.cell_list.splice(2,2);
            }
          }
        });
        ...
        this.$http.get("api/my_paylist").then((data)=>{
          ...
          this.cell_list[3].url=tag===1?"wallet_code?mark=2":"open_wallet?type=1";
        })
      }
    },
    methods:{
