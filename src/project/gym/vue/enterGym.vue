<template>
  <div class="wrap xjt_wrap finish-bg wrap_enter">
    ...
    <div class="timeBox gymBox flex_box">
  		<div class="row flex">
  			<span class="dec">室内温度</span>
	    	<span class="time time_ys"><i v-text="temperature">28</i><i class="small">℃</i></span>
  		</div>
  		<div class="row flex">
  			<span class="dec">室内湿度</span>
	    	<span class="time time_ys"><i v-text="humidity">30</i><i class="small">%</i></span>
  		</div>
  		<div class="row flex">
  			<span class="dec">PM2.5</span>
	    	<span class="time time_ys" v-text="pm">70</span>
  		</div>
  		<div class="row flex">
  			<span class="dec">TVOC</span>
	    	<span class="time time_ys"><i v-text="tvoc">0.7</i><i class="small">mg/m³</i></span>
  		</div>
  	</div>
    <!--detail end-->  
    <!--down start-->
	  <div class="down flex_box">
	  	<div class="flex flex_box horizontal vertical">
	  		<a @click="goLeave()" class="goBack flex_box horizontal vertical" href="javascript:void(0)">离开</a>
	  	</div>
	  	<div class="flex flex_box horizontal vertical">
	  		<a @click="goOpenEquipment()" class="goPay goGym flex_box horizontal vertical" href="javascript:void(0)">
	  		  <div class="circle flex_box horizontal vertical">扫描器材</div>
	  		</a>
	  	</div>
	  	<div class="flex flex_box horizontal vertical">
	  		<a @click="openDoor()" class="goBack aGreen flex_box horizontal vertical" href="javascript:void(0)">开门</a>
	  	</div>
	  </div>
	  <!--down end-->
  </div>
...
export default {
  ...
  methods: {
    ...
    goLeave: function () { // 离开健身房,进入缴费
      gym.goExit(this.tocken, {
         ...
         if (res.status === 200) {
              ...
              this.starttime = this.Xtimestamp(data.starttime) // 通过换算得到的时间
              this.endtime = this.Xtimestamp(data.endtime) // 通过换算得到的时间
    ...
    getParaData: function () { 
       ...
       // 将得到的开始时间存储起来
       this.saveValue("returnStarttime",this.starttime,24)
       // 温度、湿度、pm、tvoc
      this.temperature = this.getValue("temperature") === '' ? 0 :  this.getValue("temperature")
      this.humidity = this.getValue("humidity") === '' ? 0 :  this.getValue("temperature")
      this.pm = this.getValue("pm") === '' ? 0 :  this.getValue("temperature")
      this.tvoc = this.getValue("tvoc") === '' ? 0 :  this.getValue("temperature")
    },
    update2 () {
      this.shows ++;
