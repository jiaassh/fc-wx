    ...
    <div>
          ...
          <div>
            ...
            <div v-if="index==0">
              <div class="tw_detail">
                ...
                <div style='font-size:0'><img v-for="item in detailimgs" :src="item" class="object_fit" style="float:left" /></div>
              </div>
            </div>
            <div v-else>
              <div class="pj_detail" v-for="item in evaluate">
                <div class="f_flex" style="font-size:13px;">
                  ...
                  <div class="flex_1">
                    ...
                    <div class="text_ellipsis" style="color:#999;width:239px;">{{item.eva_time}}<span style="margin-left:.5em;">{{item.model}}</span></div>
                    <div style="margin-top:-5px;">{{item.eva_content}}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-transfer-dom>
          <popup v-model="popup_model" position="bottom" style="background-color:white">
            ...
            <div style="display:inline-block;vertical-align: top;padding:10px;font-size:13px">
              <div style="font-size:14px;color:red">￥{{select_price?select_price.toFixed(2):min_price+'~'+max_price}}</div>
              <div>库存{{select_stock?select_stock:all_stock}}件</div>
            ...
            <div style="padding:0 0 10px;margin:0 10px;">
              ...
              <!-- 加的 -->
              <div v-show='showing&&roundValue>=shownumname' style="font-size:0.5rem"><span>赠送</span><span style="color:#f00" v-text="numgiven"></span>箱</div>
            ...
            <div v-else class="f_flex" style="margin-top:10px;">
              <a class="common_button flex_1 color_orangered" @click="addCar">加入购物车</a>
              <a class="common_button flex_1 color_orange" @click="goBuy">立即下单</a>
            </div>
          </popup>
        </div>
      </div>
      <div class="fixed_bottom">
        <div class="weui_flex" style="padding:0;text-align:center;height:44px;">
          <a style="flex:2;border-right:1px solid #ececec;"><router-link :to="{path:'store_detail',query: {store_id:store_id}}"><div style="height: 22px"><img style="height: 25px" src="./店铺.png"/></div></router-link></a>
          <a :href="'tel:'+tel" style="flex:2;border-right:1px solid #ececec;"><div style="height: 22px"><img style="height: 25px" src="./电话.png"/></div></a>
          <a v-if="trolley===0" style="flex:2;border-right:1px solid #ececec;" @click="collect"><div style="height: 22px"><img style="height: 25px" src="./收藏.png"/></div></a>
          <a v-else style="flex:2;border-right:1px solid #ececec;" @click="cancelcollect"><div style="height: 22px"><img style="height: 25px" src="./星星色.png"/></div></a>
          <a @click="showActionsheet(2)" class="cz_button color_orangered" style="flex:4;color:white">加入购物车</a>
          <a @click="showActionsheet(1)" class="cz_button color_orange" style="flex:4;background-color:#e9582a;color:white">立即下单</a>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    if (service_url) {
      ...
      this.$http
        .post(service_url + "/o2o/shop/wx/goodinfo.do", senddata)
        .then(data => {
          ...
          for (let i in self.models) {
            self.all_stock += parseInt(self.models[i].stock);
          }
      ...
      this.$http.get("/api/goodsDetails").then(
        data => {
          ...
          for (let i in self.evaluate) {
            self.evaluate[i].eva_time = this.getLocalTime(
              self.evaluate[i].eva_time
            );
          }
  ...
  methods: {
    ...
    change(val) {
      ...
      var count = Math.floor(this.roundValue / this.shownumname);
      console.log(count, "count");
      if (count) {
        this.numgiven = count * this.singnum;
      }else{
        this.numgiven=0
      }
    },
    itemclick(val) {
      ...
      for (let i = 0; i < this.models.length; i++) {
        if (val === this.models[i].name) {
          ...
          this.select_price = parseFloat(this.models[i].price);
          this.model_id = this.models[i].modelid;
          if (this.models[i].activityType) {
            this.showing = true;
            this.shownumname = this.models[i].cardinalNumber;
            this.singnum = this.models[i].discountsNum;
            var count = Math.floor(this.roundValue / this.shownumname);
    ...
    confirm_buy(num) {
      ...
      if (num !== 3 && !this.choose_item) {
        ...
        if (this.click_button === 1 || num === 1) {
          ...
          if (service_url) {
            this.$http
              .post(service_url + "/o2o/shop/wx/downorder.do", senddata)
              .then(data => {
                ...
                if (data.body.status === 0) {
                  ...
                  this.$router.push({
                    path: "goods_order",
                    query: { type: 1, ordernum: ordernums }
                  });
                } else {
                  this.dialog_title = "提示";
          ...
          if (service_url) {
            this.$http
              .post(service_url + "/o2o/shop/wx/insertcar.do", insertData)
              .then(data => {
                this.isLoading = false;
                if (data.body.status === 0) {
                  this.show = true;
          ...
          if (service_url) {
            this.$http
              .post(
                service_url + "/o2o/shop/wx/insertcollection.do",
                collectData
              )
              .then(data => {
                this.isLoading = false;
...
.color_orangered {
  /*background-color: orangered;*/
  background: url("../../../static/img/store.png");
