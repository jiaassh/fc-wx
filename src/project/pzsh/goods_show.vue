  ...
  <div>
      <div :key="item" v-for="item in goods">
        <!-- <div class="weui_flex border_bottom_2" @click="showDetail(item.goods_id||item.order_id||item.id||item.ordernum,item.state)"> -->
          <!-- 新改的 -->
        <div class="weui_flex border_bottom_2" @click="showDetail(item.goods_id||item.order_id||item.id||item.ordernum,item.state,item.status)">
          <div class="img_div">
          ...
          <div class="ui_describe flex_1 posr font_14">
            <template v-if="type === '1'">
              ...
              <div class="text_ellipsis width_13"><span style="margin-right:5px;">数量：{{item.num}}</span>规格：{{item.modelname}}</div>
              <!-- 加的 -->
              <div v-show='item.activityType&&item.giveCount' class="text_ellipsis width_13" style="font-size:0.45rem;color:#f00">赠送: {{item.giveCount}}</div>
            ...
            <template v-if="type === '3'">
              ...
              <x-button mini plain type="primary" style="font-size:10px;position:absolute;right:0;bottom:0" @click.native="onDeleteClick(item.goods_id,event)">取消收藏</x-button>
            </template>
            <template v-if="type === '4'">
              <div style="max-height:47px;font-size:17px;overflow: hidden"><span style="font-size:13px;padding:1px 5px;border-radius:5px;border:1px solid #ad7de7;color:#ad7de7;margin-right:5px;vertical-align: baseline;">{{item.state_name}}</span>{{item.name}}</div>
              <div class="text_ellipsis width_13">{{item.goods_date}}</div><span style="float:right;">{{item.quality}}</span>
              <div class="color_red font_1_4" style="position:absolute;left:0;bottom:0">￥{{item.price.toFixed(2)}}</div>
            ...
            <template v-if="type === '5'">
              ...
              <div><div class="text_ellipsis" style="width:140px;">{{item.project_name}}</div><span style="float:right;color:#ad7de7">{{item.state_name}}</span></div>
              <div>场地: {{item.area_name}}</div>
              <div>日期: {{item.area_date}}</div>


              <!-- 加的 -->
              <div>预约时间: {{item.area_time}}</div>
            ...
            <template v-if="type === '6'">
              <div><div class="text_ellipsis" style="width:140px;">{{item.name}}</div><span style="float:right;color:#ad7de7">{{item.state_name}}</span></div>
              <div>订单号: {{item.order_num}}</div>
              <div>期望日期: {{item.hope_reserve}}</div>
              <div v-if="this.route_type == 1" class="color_red font_1_4" style="position:absolute;left:0;bottom:0">￥{{item.price}}</div>
...
export default {
  ...
  methods: {
    ...
    onConfirm() {
      switch (this.mark) {
        case 1:
          let url =
            service_url + "/o2o/flea/wx/shelves?orderid=" + this.this_id;
          let this_url =
            this.status === 1 ? url + "&shelvestype=1" : url + "&shelvestype=0";
        ...
        case 2:
          if (service_url) {
            this.$http
              .get(service_url + "/o2o/flea/wx/shelf?orderid=" + this.this_id)
              .then(data => {
                if (data.body.result.status === 0) {
                  this.show_toast = true;
          ...
          if (service_url) {
            ...
            if (this.type === "6") {
              if (this.route_type == 1) {
                ...
                get_url =
                  service_url + "/o2o/reno/wx/cancel?id=" + this.this_id;
              }
    ...
    showDetail(order_id, state, status) {
      if (this.type === "1") {
        /*this.$router.push({
              path:this.path,
      ...
      if (this.type === "4") {
        console.log(order_id, "state1");
        this.$router.push({
          path: this.path,
          query: { goods_id: order_id, state: state, status: status }
        });
      }
      if (this.type === "5") {
        this.$router.push({
  ...
  //     var imgs=value.img?value.img:0
  //     var imgs1=value.goods_img?value.goods_img:0
  //     var imgs2=value.area_img?value.area_img:0
  //     if(imgs!=0){
  //       value.img=value.img.split(',')[0]
  //     }else if(imgs1!=0){
  //       value.goods_img=value.goods_img.split(',')[0]
  //     }else if(imgs2!=0){
  //       value.area_img=value.area_img.split(',')[0]
  //     }
