    ...
    <div v-else>
        ...
        <Group>
          <div class="body_white" style="border-bottom:1px solid #ececec" v-for="(order,index1) in orders">
            ...
            <div  class="weui_flex border_bottom_2" v-for="(item,index2) in order.goods">
              ...
              <div @click="go_detail(item.goods_id)" style="margin-left:20px" class="flex_1 font_14 posr">
                <div style="max-height:40px;overflow: hidden">{{item.name}}</div>
                <div class="text_ellipsis width_13">规格: {{item.modelname}}</div>
                <div style="display:-webket-box">
                  <div class="text_ellipsis width_13">数量：<span class="mark_span">X</span><span
                  class="num_span">{{item.num}}</span></div>
                  <div v-show='item.activityType&&item.giveCount' class="text_ellipsis width_13" style="color:#f00">赠送: <span v-text="item.giveCount"></span></div>
                </div>
                <div class="color_red" style="left:0;bottom:0"><span class="mark_span">￥</span><span class="price_span" style="font-size:20px;">{{item.price}}</span></div>
      ...
      <div class="fixed_bottom">
        <div class="weui_flex">
          ...
          <div class="flex_1">
            <span>全选</span>
            <p style="float:right">合计: <span class="color_red"> ￥{{sum.toFixed(2)}}</span></p>
          </div>
          <div class="buy_button" @click="placeOrder">下单</div>
          <!-- <div class="buy_button" @click="confirorder">下单</div> -->
        </div>
      </div>
    </div>
...
//              this.$router.push({ path: '/store_up',query:{"type":1} });
    ...
    methods:{
      confirorder(){
        this.$router.push('/store_confirm')
      },
...
//        this.checkPrice();
      ...
      checkPrice(){
        ...
        console.log(checked_obj.length);
        console.log($(".weui_check_single").length);
        if(checked_obj.length===$(".weui_check_single").length){
          this.check=false;
          $("#checkboxall").prop("checked","true");
        }
        this.sum=0;
        ...
        for(let k = 0;k<checked_obj.length;k++){
          ...
          let price = parseFloat(siblings.find(".price_span").html());
